...
def get_all_bookings(email):
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """SELECT regno, name, EXTRACT(DAY FROM starttime) AS date, EXTRACT(hour FROM starttime) AS hour
                 FROM carsharing.car LEFT OUTER JOIN carsharing.booking ON (car = regno) LEFT OUTER JOIN carsharing.member ON (madeby = memberno)
                 WHERE email=%s"""
...
def get_booking(b_date, b_hour, car):
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """SELECT nickname, regno, car.name, EXTRACT(DAY FROM starttime) AS date, EXTRACT(hour FROM starttime) AS hour, (EXTRACT(hour FROM endtime) - EXTRACT(hour FROM starttime)) AS duration, whenbooked
                FROM carsharing.car LEFT OUTER JOIN carsharing.booking ON (regno = car) LEFT OUTER JOIN carsharing.member ON (madeby = memberno)
